...
- [x] Add root `docker-compose.yml`: `tpt-daemon` + real `otel-collector` + viewer, pre-wired to the example config
- [x] Add `tpt-daemon --validate-config`/`--check` subcommand (dry-run config + schema load, no socket binds)
- [x] Add `tpt-daemon --version`/`--help` flags (hand-rolled, no new dependency)
- [x] Add a send-test-log CLI helper to fire sample syslog lines at a running daemon (pairs with `examples/`)

...
use opentelemetry_proto::tonic::logs_service::client::LogsServiceClient;
use opentelemetry_proto::tonic::logs_service::ExportLogsServiceRequest;
...
fn lr_to_proto(lr: &ModelLr) -> PlLr {
    PlLr {
        time_unix_nano: lr.time_unix_nano.parse::<u64>().unwrap_or(0),
...
fn payload_to_proto(p: &LogsPayload) -> ExportLogsServiceRequest {
    let resource_logs = p
        .resource_logs
        .iter()
        .map(|rl: &ModelRl| PlRl {
            ...
            scope_logs: rl
                ...
                .map(|sl: &ModelSl| PlSl {
                    ...
                    log_records: sl.log_records.iter().map(lr_to_proto).collect(),
                    ..Default::default()
                })
                .collect(),
            ..Default::default()
